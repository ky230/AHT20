...
def chiller_on():
    ...
    proc = Popen(['python3','/home/cmsdaq/DAQ/qaqc_jig/python/utilities/setBoxTemp_PID.py','--target','%f'%(float(chiller_temp_entry.get()))])
...
def chiller_off():
    ...
    out = Popen(['/home/cmsdaq/DAQ/LAUDAChiller/driveChiller.py', '--power-off'],stdout=subprocess.PIPE)
...
def take_data():
   ...
   run = -1
   with open('/home/cmsdaq/DAQ/qaqc_jig/.last_run') as f:
      runs = [int(x) for x in next(f).split()]
      run = str(runs[0]+1)
      os.remove('/home/cmsdaq/DAQ/qaqc_jig/.last_run')
      with open('/home/cmsdaq/DAQ/qaqc_jig/.last_run','w+') as f:
         f.write(run)
   ...
   if stepper_enable.get():
      out = Popen(['cat','/home/cmsdaq/DAQ/qaqc_jig/.step_lastposition'],stdout=subprocess.PIPE)
      step_lastposition = int(out.stdout.read().decode().rstrip())
      os.system('echo %d > /home/cmsdaq/DAQ/qaqc_jig/.step_lastposition'%step_lastposition)
