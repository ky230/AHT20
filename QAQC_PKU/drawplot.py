            ...
            if sensor.lower() == 'average':
                ...
                if average_files:
                    ...
                    if not avg_data.empty:
                        axs[0].plot(avg_data['Timestamp'], avg_data['Temperature(°C)'], linestyle='-', marker=None, label=f'{sensor} Temperature')
                        axs[1].plot(avg_data['Timestamp'], avg_data['Humidity(%)'], linestyle='-', marker=None, label=f'{sensor} Humidity')
            else:
                # 查找并绘制指定传感器数据
                ...
                if sensor_files:
                    ...
                    if not sensor_data.empty:
                        axs[0].plot(sensor_data['Timestamp'], sensor_data['Temperature(°C)'], linestyle='-', marker=None, label=f'{sensor} Temperature')
                        axs[1].plot(sensor_data['Timestamp'], sensor_data['Humidity(%)'], linestyle='-', marker=None, label=f'{sensor} Humidity')
